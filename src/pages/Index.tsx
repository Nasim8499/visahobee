...
const CheckIcon = () => (
  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#003B73" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
);
...
const CountryCard = ({ img, country, visa, badge, desc, slug }: { img: string; country: string; visa: string; badge?: string; desc: string; slug: string; delay?: number; featured?: boolean; flag?: string; region?: string }) => (
  <Link to={`/countries/${slug}`} className="block bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700 group hover:-translate-y-1">
    ...
    <div className="p-3 sm:p-4">
      ...
      <div className="flex items-center gap-1 text-[#003B73] dark:text-[#177BBB] text-[9px] sm:text-xs font-semibold">
        <span>View Details</span><ArrowIcon />
      </div>
    </div>
  </Link>
);

const ServiceCard = ({ img, title, desc }: { img: string; title: string; desc: string; delay?: number }) => (
  <Link to="/services" className="block bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700 group hover:-translate-y-1">
...
const TestimonialCard = ({ name, role, text }: { name: string; role: string; text: string; delay?: number }) => (
...
export default function Index() {
  ...
  const [eligForm, setEligForm] = useState({ name: '', destination: '', visaType: '' });
  ...
  const inputCls = `w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#003B73] transition-colors ${isDark ? 'bg-gray-800 border-gray-700 text-white placeholder:text-gray-500' : 'bg-white border-gray-200 text-gray-900'}`;
  const selectCls = `w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#003B73] transition-colors ${isDark ? 'bg-gray-800 border-gray-700 text-gray-400' : 'bg-white border-gray-200 text-gray-500'}`;
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-950' : 'bg-[#F5F5F0]'}`}>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${pastHero ? (isDark ? 'bg-gray-900/95 backdrop-blur-xl shadow-sm' : 'bg-white/95 backdrop-blur-xl shadow-sm') : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14 sm:h-16">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {navLinks.map(item => (
              <button key={item} onClick={() => handleNav(item)} className={`text-sm font-medium transition-colors ${pastHero ? (isDark ? 'text-gray-400 hover:text-[#177BBB]' : 'text-gray-600 hover:text-[#003B73]') : 'text-white/80 hover:text-white'}`}>{item}</button>
            ))}
          </nav>
          <div className="flex items-center gap-2">
        ...
        {mobileMenu && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm" onClick={() => setMobileMenu(false)}>
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className={`absolute left-0 top-0 bottom-0 w-72 ${isDark ? 'bg-gray-900' : 'bg-white'} p-6`} onClick={e => e.stopPropagation()}>
              ...
              {navLinks.map(item => (
                <button key={item} onClick={() => handleNav(item)} className={`block w-full text-left py-3 text-sm font-medium transition-colors border-b ${isDark ? 'text-gray-300 hover:text-[#177BBB] border-gray-800' : 'text-gray-700 hover:text-[#003B73] border-gray-100'}`}>{item}</button>
              ))}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className={`${isDark ? 'bg-gray-900' : 'bg-white'} rounded-3xl p-6 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto`} onClick={e => e.stopPropagation()}>
              {formSubmitted ? (
                <div className="text-center py-8">
                  ...
                  <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>We'll contact you within 24 hours.</p>
                </div>
              ) : (
                <>
                  ...
                  <form onSubmit={e => { e.preventDefault(); setFormSubmitted(true); }} className="space-y-4">
                    <input placeholder="Full Name" className={inputCls} required />
                    <input placeholder="Email Address" type="email" className={inputCls} required />
                    <input placeholder="Phone Number" className={inputCls} />
                    <select className={selectCls}><option value="">Select Service</option><option>Work Permit</option><option>Tourist Visa</option><option>Business Visa</option><option>Recruitment</option></select>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => { setShowEligibility(false); setEligResult(null); }}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className={`${isDark ? 'bg-gray-900' : 'bg-white'} rounded-3xl p-6 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto`} onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                <input value={eligForm.name} onChange={e => setEligForm({ ...eligForm, name: e.target.value })} placeholder="Full Name" className={inputCls} />
                <select value={eligForm.destination} onChange={e => setEligForm({ ...eligForm, destination: e.target.value })} className={selectCls}>
                  <option value="">Destination Country</option>
                  {countries.map(c => <option key={c.slug}>{c.country}</option>)}
                </select>
                <select value={eligForm.visaType} onChange={e => setEligForm({ ...eligForm, visaType: e.target.value })} className={selectCls}>
                  <option value="">Visa Type</option><option>Work Permit</option><option>Visitor Visa</option><option>Business Visa</option><option>Skilled Migration</option>
                </select>
                ...
                {eligResult && (
                  <div className={`rounded-xl p-4 border ${isDark ? 'bg-[#177BBB]/10 border-[#177BBB]/20' : 'bg-[#003B73]/10 border-[#003B73]/20'}`}>
      ...
      <section id="home" className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        {[{ x: 65, y: 35 }, { x: 82, y: 68 }, { x: 50, y: 22 }, { x: 56, y: 30 }, { x: 72, y: 45 }].map((dot, i) => (
        ...
        <div className="relative z-20 text-center px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>
            ...
            <p className="text-white/40 text-xs sm:text-sm max-w-md mx-auto mt-3 sm:mt-4">{slide.desc}</p>
          </motion.div>
      ...
      <section id="countries" className="pt-12 sm:pt-16 pb-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <p className={`text-center text-xs sm:text-sm mb-6 sm:mb-8 max-w-xl mx-auto ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Comprehensive work visa support across 10 countries.</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-5">
            {filteredCountries.map((c) => <CountryCard key={c.slug} {...c} />)}
          </div>
        </div>
      </section>
      ...
      <section id="services" className={`pt-12 sm:pt-16 pb-12 sm:pb-16 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <p className={`text-center text-xs sm:text-sm mb-8 max-w-xl mx-auto ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Comprehensive visa and recruitment solutions.</p>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
            {serviceData.map((s) => <ServiceCard key={s.title} {...s} />)}
          </div>
          <div className="text-center mt-8">
            <Link to="/services" className={`inline-flex items-center gap-2 text-sm font-semibold ${isDark ? 'text-[#177BBB]' : 'text-[#003B73]'} transition-colors`}>View All Services <ArrowIcon /></Link>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <p className={`text-sm leading-relaxed mb-4 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>VisaHOBe Pte. Ltd. is a Singapore-registered visa and recruitment support company — a complete "End-to-End" visa travel partner across 10 countries.</p>
              <Link to="/about" className={`inline-flex items-center gap-2 text-sm font-semibold ${isDark ? 'text-[#177BBB]' : 'text-[#003B73]'}`}>Learn More <ArrowIcon /></Link>
            </div>
      ...
      <section className="pt-12 sm:pt-16 pb-12 sm:pb-16" id="contact">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={() => setShowEligibility(true)} className="bg-[#003B73] text-white rounded-full px-6 sm:px-8 py-3 text-sm font-bold shadow-lg inline-flex items-center justify-center gap-2">Check Eligibility <ArrowIcon /></button>
            <button onClick={() => setShowContactForm(true)} className={`${isDark ? 'bg-gray-800' : 'bg-gray-900'} text-white rounded-full px-6 sm:px-8 py-3 text-sm font-bold shadow-lg inline-flex items-center justify-center gap-2`}>Contact Us <ArrowIcon /></button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="border-t border-gray-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex gap-4">
              <span className="text-gray-500 text-[10px] sm:text-xs hover:text-gray-300 cursor-pointer">Privacy Policy</span>
              <span className="text-gray-500 text-[10px] sm:text-xs hover:text-gray-300 cursor-pointer">Terms of Service</span>
            </div>
          </div>
        </div>
      </footer>
