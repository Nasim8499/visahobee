    ...
    <div className="min-h-screen bg-[#F5F5F0]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <h2 className="text-2xl font-bold text-gray-900 mb-4 font-heading">Overview</h2>
              <p className="text-gray-600 text-sm sm:text-base leading-relaxed">{country.overview}</p>
            </motion.div>

            {/* Visa Types & Fees */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              <h2 className="text-2xl font-bold text-gray-900 mb-4 font-heading">Visa Types & Fees</h2>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="text-2xl font-bold text-gray-900 mb-4 font-heading">Required Documents</h2>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              <h2 className="text-2xl font-bold text-gray-900 mb-4 font-heading">Key Industries</h2>
          ...
          <div className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-gray-900 rounded-2xl p-6 text-center sticky top-24">
              ...
              <p className="text-gray-400 text-xs mb-5">Get expert guidance for your visa application. Free initial consultation.</p>
            ...
            <div>
              ...
              <div className="space-y-3">
                {otherCountries.map(c => (
                  <Link key={c.slug} to={`/countries/${c.slug}`} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
      ...
      {showContact && (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => { setShowContact(false); setFormSubmitted(false); }}>
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            {formSubmitted ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 text-sm">Your inquiry for {country.country} has been submitted. We'll contact you within 24 hours.</p>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={(e) => { e.preventDefault(); setFormSubmitted(true); }} className="space-y-4">
                  ...
                  <select className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500 transition-colors text-gray-500" defaultValue={country.visaTypes[0]?.name}>
                    {country.visaTypes.map(vt => <option key={vt.name}>{vt.name}</option>)}
                  </select>
                  <textarea placeholder="Additional details about your application..." rows={3} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500 transition-colors resize-none" />
